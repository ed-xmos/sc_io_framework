...
#define ADC_READ_INTERVAL   (50 * XS1_TIMER_KHZ)    // Time in between individual conversions in milliseconds
            ...
            adc_timer_event:
            {
                switch(adc_state){
                    case ADC_IDLE:
                        ...
                        /*     for(unsigned i = 0; i < num_ports; i++){
                            port_out(p_adc[i], 1); // Drive a 1 to charge the capacitor
                        } */
                    ...
                    case ADC_CHARGING:
                        port_in(p_adc[p_adc_idx]); // Make input and start discharge
